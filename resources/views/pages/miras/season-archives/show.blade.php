...
@php
    $record = $season ?? $seasonArchive ?? $item ?? null;
    \Carbon\Carbon::setLocale('tr');
@endphp

@section('title', ($pageTitle ?? ($record?->title ?? 'Sezon')) . ' - Cadde1905')
...
@if ($record)
    <nav class="mb-6 flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] opacity-40 px-6 lg:px-0 max-w-[840px] mx-auto">
        ...
        <a href="{{ route('miras.seasons.index') }}">Sezonlar</a> <span>/</span>
        <span class="text-red-600">Arşiv</span>
    </nav>
@endif
@endsection

@section('content')
<div class="ui-detail-page ui-detail-page--season">
    ...
    @if ($record)
        @php
            $mainImage = $record->hasCoverImage()
                ? $record->coverImageUrl()
                : asset('images/placeholders/miras-placeholder.webp');

            $wordContent = (string) ($record->summary . ' ' . $record->content . ' ' . $record->season_overview);
            $wordCount = str_word_count(strip_tags($wordContent));
            $readingTime = max(1, ceil($wordCount / 200));
            $startEnd = trim((string) $record->start_year . ' - ' . ($record->end_year ?: '?'));

            // MEDIA
            $galleryItems = $record->galleryMedia()->orderByPivot('sort_order')->get();
            $videoItem = $record->videoMedia()->orderByPivot('sort_order')->first();
            $allGalleryImages = $galleryItems
                ->map(fn ($media) => $media->url ?: ($media->path ? asset('storage/' . ltrim($media->path, '/')) : null))
                ->filter()
                ->values();

            $seasonVideoEmbedUrl = null;
            if ($videoItem && ! empty($videoItem->embed_url)) {
                ...
                if (str_contains($url, 'youtu.be')) {
                    ...
                    if ($id) $seasonVideoEmbedUrl = "https://www.youtube.com/embed/$id";
                } elseif (str_contains($url, 'youtube.com/watch')) {
                    parse_str(parse_url($url, PHP_URL_QUERY), $q);
                    if (isset($q['v'])) $seasonVideoEmbedUrl = "https://www.youtube.com/embed/{$q['v']}";
                } else {
                    $seasonVideoEmbedUrl = $url;
                }
            }
        @endphp

        <header class="ui-detail-hero">
            <div class="ui-detail-hero-media"><img src="{{ $mainImage }}" alt="{{ $record->title }}"></div>
            ...
            <div class="ui-detail-content">
                ...
                <div class="ui-detail-meta">
                    <span class="text-amber-500">{{ $record->season_label }}</span>
        ...
        <main class="ui-detail-layout px-6 lg:px-0">
            ...
            <div class="ui-detail-facts ui-detail-facts--season">
                @if($record->manager_name)
                    <div class="ui-detail-fact-card">
                        <span class="ui-detail-fact-label">Teknik Direktör</span>
                        <span class="ui-detail-fact-value">{{ $record->manager_name }}</span>
                    </div>
                @endif
                <div class="ui-detail-fact-card">
                    <span class="ui-detail-fact-label">Dönem</span>
                    <span class="ui-detail-fact-value">{{ $startEnd }}</span>
                </div>
                @if(!empty($record->season_label))
                    <div class="ui-detail-fact-card">
                        <span class="ui-detail-fact-label">Etiket</span>
                        <span class="ui-detail-fact-value">{{ $record->season_label }}</span>
                    </div>
                @endif
            </div>

            @if(!empty($record->content))
                <x-rich-content class="ui-detail-body" :html="$record->content" />
            @else
                <article class="ui-detail-body">
                    <div class="p-16 text-center border border-zinc-800 rounded-3xl bg-zinc-900/10 italic opacity-50">
                        Bu sezon arşivi için henüz detay metni girilmedi.
                    </div>
                </article>
            @endif

            @if($seasonVideoEmbedUrl)
                <div class="ui-detail-media rounded-3xl overflow-hidden shadow-2xl bg-black aspect-video">
                    <iframe src="{{ $seasonVideoEmbedUrl }}" class="w-full h-full border-0" allowfullscreen></iframe>
                </div>
            @endif
            ...
            @if($galleryItems->isNotEmpty())
                <div class="ui-detail-media">
                    <div class="flex items-center justify-between mb-10">
                        <h2 class="text-2xl font-black m-0">SEZON GALERİSİ</h2>
            ...
            @if(!empty($record->season_overview))
                <section class="ui-detail-section">
                    <h2 class="ui-detail-section-title">Sezonun Özeti</h2>
                    <div class="ui-detail-section-copy">{!! nl2br(e($record->season_overview)) !!}</div>
                </section>
            @endif

            @if(!empty($record->league_summary))
                <section class="ui-detail-section">
                    <h2 class="ui-detail-section-title">Lig Mücadelesi</h2>
                    <div class="ui-detail-section-copy">{!! nl2br(e($record->league_summary)) !!}</div>
                </section>
            @endif

            @if(!empty($record->europe_summary))
                <section class="ui-detail-section">
                    <h2 class="ui-detail-section-title">Avrupa Arenası</h2>
                    <div class="ui-detail-section-copy">{!! nl2br(e($record->europe_summary)) !!}</div>
                </section>
            @endif

            @if(!empty($record->cup_summary))
                <section class="ui-detail-section">
                    <h2 class="ui-detail-section-title">Kupa Dosyası</h2>
                    <div class="ui-detail-section-copy">{!! nl2br(e($record->cup_summary)) !!}</div>
                </section>
            @endif

            <div class="ui-detail-page-end mt-24 pt-10 border-t border-zinc-800 flex justify-center pb-20">
                <a href="{{ route('miras.seasons.index') }}" class="px-12 py-5 bg-zinc-900 border border-zinc-800 text-white rounded-full font-black text-xs uppercase tracking-[0.3em] hover:bg-red-700 hover:border-red-700 transition duration-300 shadow-2xl">
                    ← TÜM SEZONLAR
                </a>
            </div>
        </main>
    @else
        <div class="py-20 text-center opacity-50 italic">Kayıt bulunamadı.</div>
    @endif
</div>

@if($record && $galleryItems->isNotEmpty())
...
<script>
    (function() {
        ...
        const imgs = @json($record ? $allGalleryImages : []);
        if (imgs.length) {
