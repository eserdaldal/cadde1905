...
@section('worldcup-content')
    ...
    <section class="relative overflow-hidden pt-16 pb-20 border-b border-[var(--border-default)]">
        <div class="absolute inset-0 bg-gradient-to-br from-[#8D1B3D]/10 via-[var(--surface-base)] to-[#1D6F42]/10"></div>
        ...
        <div class="wc-container relative">
            ...
            <div class="flex flex-col lg:flex-row items-center justify-center gap-10 lg:gap-16">
                ...
                <div class="text-center group flex-1 min-w-[220px]">
                    <div class="w-24 h-24 sm:w-32 sm:h-32 bg-white/5 rounded-full overflow-hidden border-4 border-white/5 mx-auto mb-4 shadow-2xl transition-transform group-hover:scale-105">
                    ...
                    <h2 class="text-white font-black text-xl sm:text-2xl tracking-tight group-hover:text-[var(--accent-premium)] transition-colors max-w-[260px] mx-auto truncate">{{ $homeName }}</h2>
                    <p class="text-[var(--text-muted)] text-[10px] font-bold uppercase tracking-widest mt-1">Ev Sahibi</p>
                </div>

                {{-- SCORE --}}
                <div class="relative flex flex-col items-center">
                    <div class="wc-score-panel flex items-center gap-4 px-8 py-4 bg-black/40 backdrop-blur-xl rounded-3xl border border-white/10 shadow-2xl">
                        <span class="text-white font-black text-5xl sm:text-7xl lg:text-8xl tabular-nums leading-none">{{ $scoreHome ?? '–' }}</span>
                        <span class="text-[var(--accent-premium)] text-2xl sm:text-4xl font-black">:</span>
                        <span class="text-white font-black text-5xl sm:text-7xl lg:text-8xl tabular-nums leading-none">{{ $scoreAway ?? '–' }}</span>
                    </div>
                ...
                <div class="text-center group flex-1 min-w-[220px]">
                    ...
                    <h2 class="text-white font-black text-xl sm:text-2xl tracking-tight group-hover:text-[var(--accent-premium)] transition-colors max-w-[260px] mx-auto truncate">{{ $awayName }}</h2>
            ...
            <div class="text-center mt-8 flex flex-col items-center gap-2">
                ...
                <div class="text-[var(--text-muted)] text-xs font-medium bg-white/[0.03] px-4 py-1.5 rounded-full border border-white/5">
                    @if ($stadiumName)
    ...
    <div class="wc-container">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <section class="wc-section wc-section--compact">
                ...
                <div class="wc-card p-7 bg-gradient-to-b from-white/[0.02] to-transparent">
                    <div class="max-w-xl mx-auto space-y-6">
                        ...
                        @foreach ($matchStats as $ms)
                            ...
                            <div class="flex items-center justify-between text-sm">
                                <span class="text-white font-black tabular-nums w-20 text-left">
                                    @if ($isPlaceholder)
                                ...
                                <div class="flex-1 px-4">
                                    ...
                                    <div class="h-1.5 w-full bg-white/5 rounded-full overflow-hidden flex">
                                        ...
                                        <div class="h-full bg-white/20 {{ $isPlaceholder ? 'opacity-20' : '' }}" style="width: 50%"></div>
                                    </div>
                                </div>
                                <span class="text-white font-black tabular-nums w-20 text-right">
                                    @if ($isPlaceholder)
                                        <span class="wc-ghost-value w-10 h-4 opacity-30"></span>
                                    @else
                                        {{ $awayStat }}
                    ...
                    @foreach ([
                        ...
                        <div class="wc-card">
                            <div class="px-5 py-4 border-b border-white/5 bg-white/[0.02]">
                                <h3 class="text-white font-black text-sm uppercase tracking-tight">{{ $side['label'] }}</h3>
                            </div>
                            <div class="p-5 space-y-3">
                                @foreach ($side['items']->take(11) as $idx => $player)
                                    ...
                                    <div class="flex items-center gap-4 text-xs font-bold transition-all hover:translate-x-1 group">
                                        <span class="w-8 h-8 rounded-lg bg-white/5 border border-white/5 flex items-center justify-center text-[var(--accent-premium)] tabular-nums group-hover:bg-[var(--accent-premium)] group-hover:text-[#101010] transition-colors">{{ $idx + 1 }}</span>
                                        <span class="text-[var(--text-secondary)] group-hover:text-white transition-colors">{{ $playerName }}</span>
        ...
        <section class="wc-section wc-section--compact pt-0 pb-24">
            <div class="wc-card wc-card--highlight p-6 flex flex-col md:flex-row items-center gap-6 border border-[var(--accent-premium)]/30 bg-white/[0.02] shadow-2xl">
                 ...
                 <div class="text-center md:text-left flex-1">
                    <h3 class="text-white font-black text-2xl tracking-tight">
                        @if ($stadiumName)
                            {{ $stadiumName }}
                        @else
                 ...
                 <div class="flex-shrink-0">
                     <a href="{{ route('worldcup.stadiums.index') }}" class="px-6 py-3 rounded-full bg-white/5 border border-white/10 text-white text-xs font-black uppercase tracking-widest hover:bg-white/10 transition-all">Stadyum Detayları</a>
                 </div>
            </div>
        </section>
    </div>
